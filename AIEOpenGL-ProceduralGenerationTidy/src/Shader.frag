...
void main()	
{
		...
		vec3 N = (texture(ground_textureN,vTexCoord)*(1-texture(diffuse,vTexCoord).rrrr+textureBlend) + texture(snow_textureN,vTexCoord)*(texture(diffuse,vTexCoord).rrrr-textureBlend)).xyz * 2 - 1;
		float d1 = max(0, dot(normalize(vNormal.xyz), normalize(LightDir)));
		float d2 = max(0, dot(normalize(N), normalize(LightDir)));	
		FragColor = 0.4*(texture(ground_textureD,vTexCoord)*(1-texture(diffuse,vTexCoord).rrrr+textureBlend) + texture(snow_textureD,vTexCoord)*(texture(diffuse,vTexCoord).rrrr-textureBlend));
